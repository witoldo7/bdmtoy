...
#include "TAP/NEXUS/NEXUS_private.h"
...
static struct __attribute((packed)) {
    ...
    uint8_t   DEBUGG:1; // Debug mode. Set once the CPU is in debug mode. It is negated once the CPU exits debug mode (even during a debug session).
